...
class Kivi_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/kivi-public.css',
			array( 'slick-theme' ), $this->version, 'all' );
		wp_enqueue_style( 'slick', plugin_dir_url( __FILE__ ) . 'css/slick.css', array(), $this->version, 'all' );
		wp_enqueue_style( 'slick-theme', plugin_dir_url( __FILE__ ) . 'css/slick-theme.css', array( 'slick' ),
			$this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/kivi-public.js',
			array( 'jquery', 'slick' ), $this->version, false );
		wp_enqueue_script( 'slick', plugin_dir_url( __FILE__ ) . 'js/slick.min.js', array( 'jquery' ), $this->version,
			false );
	}
	...
	static function map_post_meta( $meta_field ) {
		...
		$arr        = array(
			...
			"encumbrance"                   => __( "Kirjatut muut rasitukset, rasitteet ja valinnanrajoitukset",
				"kivi" ),
			"possession_partition_flag"     => __( "Kirjattu hallinnonjakosopimus", "kivi" ),
			"unpaid_bills_amount"           => __( "Maksamattomat maksut", "kivi" ),
			"encumbrance_reported_flag"     => __( "Toimeksiantaja on ilmoittanut kaikki sellaiset kiinteistöön kohdistuvat…",
				"kivi" ),
			...
			"rc_expense_oblications"        => __( "Kustannuksia myöhemmin aiheuttavat velvoitteet esim. autopaikat",
				"kivi" ),
			...
			"rc_repair_need"                => __( "(Taloyhtiön) Rakennuksissa havaitut puutteellisuudet, virheellisyydet ja korjaustarpeet",
				"kivi" ),
			"rc_roof"                       => __( "Taloyhtiön Kattotyyppi", "kivi" ),
			"rc_roofing"                    => __( "Taloyhtiön Kate", "kivi" ),
			"rc_usageyear"                  => __( "Käyttöönottovuosi", "kivi" ),
			"rc_wastewater_flag"            => __( "Jätevesiasetuksen mukainen selvitys, suunnitelma sekä käyttö- ja huolto-ohje",
				"kivi" ),
			...
			"rc_mortgage_date"              => __( "(Taloyhtiön kiinnitykset) Isännöitsijän todistuksen mukaisena päivämääränä",
				"kivi" ),
			...
			"tradetype_id"                  => __( "Ilmoitustyyppi 1=myynti, 2=vuokraus, 3=myynti tai vuokraus",
				"kivi" ),
	...
	function filter_presentation_date( $value, $label, $properties ) {
		...
		if ( 'Esittelyt' == $label ) {
			...
			$date = trim( $date );
			if ( ! empty( $date ) ) {
				$date_obj = DateTime::createFromFormat( 'd.m.Y H:i',
					$date . " 23:59" ); // use late time of date in compare
			}
	...
	function filter_charges_parkingspace( $value, $label, $properties ) {
		...
		if ( count( $properties ) == 1 && isset( $properties[0] ) ) {
			...
			if ( '_charges_parkingspace' == $kivi_property->name ) {
				$value                 .= " €";
				$parkingcharge_type_id = get_post_meta( $kivi_property->post_id, '_parkingcharge_type_id',
					true ); // "kk" | "v"
